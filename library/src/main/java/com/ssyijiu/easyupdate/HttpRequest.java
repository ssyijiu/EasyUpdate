...
public class HttpRequest {
    ...
    private static boolean socketAbort = false;             // socket 中断
    ...
    private static final String ERROR_SDCARD = $.getString(R.string.error_sdcard);
    private static final String ERROR_NO_SPACE = $.getString(R.string.error_no_space);
    private static final String ERROR_FILE = $.getString(R.string.error_file);


    static void get(final String url, final RequestCallback callback) {
        get(url, null, callback);
    }


    // http://gank.io/api/data/Android/10/1
    static void get(final String url, final Map<String, String> params, final RequestCallback callback) {
        new Thread(
            new Runnable() {
    ...
    static void post(final String url, final RequestCallback callback) {
        post(url, null, callback);
    }


    static void post(final String url, final Map<String, String> params, final RequestCallback callback) {
    ...
    static void download(final String downUrl, final DownloadCallback callback) {
    ...
    static void download(final String downUrl, final String savePath, final DownloadCallback callback) {
    ...
    private static class DownloadThread implements Runnable {
            ...
            try {
                ...
                synchronized (HttpRequest.class) {
                    ...
                    if (threadRunning == 0 && allThreadStart) {
                        ...
                        // 未中断，下载完成
                        if (!interruptDownload && !socketAbort) {
                            handler.post(new Runnable() {
                                @Override public void run() {
         ...
         * 网络突然中断
         */
        private boolean isAbort(IOException e) {
    ...
    @NonNull private static File getProgressFile(int id) {
        ...
        return new File(EasyUpdate.context().getFilesDir(), fileName);
    }
}
